...
import {
    ...
    getRoundRecords,
    initializePermutationTracker,
    type OnSubmitChoice,
    type PermutationTracker,
    type RoundRecord,
    savePermutationTracker,
} from "./gameState.svelte.ts";
let { props, roundsPlayed = $bindable(0), onSubmitChoice }: {
...
// Session-level state
const permutationTracker: PermutationTracker = $state(initializePermutationTracker(props));

// Round-level state
const currentPermutation = $derived(getCurrentPermutation(permutationTracker));
...
const elapsedSeconds: number = $derived(
    Math.round((now - roundStartTime) / 1000 * 10) / 10,
);
$effect(() => {
...
function handleSubmit(e: SubmitEvent) {
    ...
    const record: RoundRecord = {
        rid: null,
        local_round_number: permutationTracker.roundsPlayed,
        choice_option_1: currentPermutation.actions[0].key,
        choice_option_2: currentPermutation.actions[1].key,
        choice_option_1_prediction: predictionChoice1,
        choice_option_2_prediction: 100 - predictionChoice1,
        player_1_role: props.playerRole,
        player_1_auth_id: props.playerAuthId,
        player_1_avatar: props.playerAvatar.name,
        player_1_chose: selectedChoice,
        player_2_avatar: currentPermutation.avatars[1].name,
        player_2_chose: null,
        outcome: null,
        time_elapsed_seconds: Math.round((Date.now() - roundStartTime) / 1000 * 10)
            / 10,
    };
    permutationTracker.roundsPlayed += 1;
    roundsPlayed = permutationTracker.roundsPlayed;
    onSubmitChoice?.({ record, permutationTracker });
    nextRound();
}
...
{#if permutationTracker.roundsPlayed < (props.maxRounds ?? Infinity)}
<div class="ccg-wrapper">
    ...
    >
        {@render gameFrame()}
        {#if props.showPayoffTable}
            {@render payoffTable()}
        {/if}
        {#if props.showPredictionControls}
...
{#snippet roundNumber()}
    <div
    class="ccg-round-number">Round {permutationTracker.roundsPlayed + 1} / {props.maxRounds === Infinity ? "∞" : props.maxRounds}</div>
{/snippet}
